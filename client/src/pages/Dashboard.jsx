...
import { Brain, TrendingUp, Clock, Plus, MessageCircle, Upload, ChevronRight, Zap } from 'lucide-react';
  ...
  useEffect(() => {
    api.get('/interviews').then(({ data }) => {
      ...
      const avgScore = completed.length
        ? (completed.reduce((a, b) => a + (b.score || 0), 0) / completed.length).toFixed(1) : 0;
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-8">
          {[
            { icon: Brain, label: 'Total', value: stats.total, color: 'text-violet-400', bg: 'bg-violet-500/10' },
            { icon: Clock, label: 'Completed', value: stats.completed, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
            { icon: TrendingUp, label: 'Avg Score', value: `${stats.avgScore}/10`, color: 'text-amber-400', bg: 'bg-amber-500/10' },
          ].map((s) => (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {quickActions.map(({ to, icon: Icon, label, desc, color, shadow }) => (
            <Link key={to} to={to}
              className={`bg-gradient-to-br ${color} ${shadow} shadow-lg p-4 sm:p-5 rounded-2xl hover:opacity-90 hover:scale-[1.02] transition-all active:scale-[0.98]`}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/history" className="text-violet-400 hover:text-violet-300 text-sm">View all →</Link>
        </div>
        ...
        {recent.length === 0 ? (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-10 sm:p-14 text-center">
            ...
            <p className="text-slate-400 text-sm mb-4">Apna pehla mock interview shuru karo!</p>
              ...
              <Link key={i.id}
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-slate-500 text-xs mt-0.5 capitalize">{i.difficulty} • {new Date(i.createdAt).toLocaleDateString('en-IN')}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-right">
                    ...
                    {i.score != null && <p className="text-white font-bold text-sm mt-1">{i.score}/10</p>}
                  </div>
